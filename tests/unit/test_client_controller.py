...
# Fixture to mock authentication for all tests that need it
...
def test_create_client_repository_error(session, mock_auth, seeded_user_commercial):
    controller = ClientController(session)
    with patch('controllers.client_controller.ClientRepository.save', side_effect=Exception('DB error')):
        with pytest.raises(CrmIntegrityError, match="Could not create client: DB error"):
...
def test_update_client_repository_error(session, mock_auth, seeded_user):
    """
