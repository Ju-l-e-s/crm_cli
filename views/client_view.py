...
class ClientsView:
    ...
    def show_menu(self):
        ...
        while True:
            options = [
                ...
                ("Edit client", self.edit_client) if self.user.role.value in (
                    "commercial", "gestion") else None,
                ("Back", lambda: "back"),
            ]
            valid_options = [opt for opt in options if opt]
            choice_idx = display_menu(
                "Clients Menu", [label for label, _ in valid_options]) - 1
    ...
    def list_clients(self):
        try:
            ...
            table = create_table(
                "All Clients", ["ID", "Name", "Email", "Phone", "Company"])
            for c in clients:
    ...
    def edit_client(self):
        ...
        fullname = self.console.input(
            f"New full name ([cyan]{client.fullname}[/cyan]): ").strip()
        email = self.console.input(
            f"New email ([cyan]{client.email}[/cyan]): ").strip()
        phone = self.console.input(
            f"New phone ([cyan]{client.phone}[/cyan]): ").strip()
        company = self.console.input(
            f"New company ([cyan]{client.company}[/cyan]): ").strip()
        update_data = {
        ...
        try:
            client = self.controller.update_client(
                client_id=int(client_id), **update_data)
